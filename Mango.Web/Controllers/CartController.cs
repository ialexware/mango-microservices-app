    ...
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(CartDto cartDto)
        {
            try
            {
                ...
                {
                    var approvedURL = $"{Request.Scheme}://{Request.Host.Value}/";
                    // Get stripe session id and redirect to checkout
                    StripeRequestDto stripeRequest = new()
                    {
                        ApprovedURL = $"{approvedURL}cart/Confirmation?orderId={orderHeaderDto.OrderHeaderId}",
                        CancelURL = $"{approvedURL}cart/checkout",
                        OrderHeader = orderHeaderDto,
                    };

                    var responseDto = await _orderService.CreateStripeSession(stripeRequest);
                    StripeRequestDto stripeRequestDto = JsonConvert.DeserializeObject<StripeRequestDto>(Convert.ToString(responseDto.Result));
